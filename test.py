...
def test_generation_GPT2BATCH(model, tokenizer, input_text, device, do_sample=False, temperature=1.0,top_k=0,top_p=0,max_length=30,task_id=-1,masks_pre=None,s=None):
        ...
        if task_id == -1:
            if masks_pre is not None:
                outputs = model(input_ids, attention_mask=attention_mask, position_ids=position_ids,
                                past_key_values=past, masks_pre=masks_pre)
            else:
                outputs = model(input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past)
        else:   # elif s:
            outputs = model(input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past, task_id=task_id, s=s)
        # else:
...
def generate_sample_prev_task(args,model,tokenizer,dataset_dic,task_id_so_far,number_of_sample,time,task_id_adpt=-1):
    ...
    temp_aug_mem = random.sample(dataset_dic[task_id_so_far], min(len(dataset_dic[task_id_so_far]), number_of_sample))
    temp_aug_sam = random.sample(dataset_dic[task_id_so_far], min(len(dataset_dic[task_id_so_far]), number_of_sample))
    with torch.no_grad():
...
def test_model_seq2seq(args,model,tokenizer,test_loader,time="0_['']",single_task=False):
    device = torch.device(f"cuda:0")
    ...
    if args.multi:
        task_id_list = test_loader.keys()
    elif args.continual:
        task_id_list = model.task_list_seen
    else:
        raise
    for task_id in task_id_list:
        if single_task and (task_id != task_id_list[-1]):
            continue
        for idx_b, batch in tqdm(enumerate(test_loader[task_id]),total=len(test_loader[task_id])):
            with torch.no_grad():
                if "gpt2" in args.model_checkpoint:
                    ...
                    responses = model.model.generate(input_ids=batch["encoder_input"].to(device),
    ...
    if single_task:
        assert args.fwt
        appendix += '_fwt'
    with open(f'{args.saving_dir}/{time}'+f'/generated_responses{appendix}.json', 'w') as fp:
...
def test_model_seq2seq_ADAPTER(args,model,tokenizer,test_loader,test_datasets,time="0_['']",use_all_masks=False,single_task=False,masks=None,TIL=False,retrain=False):  # max_seen_task=0
    # device = torch.device(f"cuda:{args.GPU[0]}")
    device = torch.device(f"cuda:0")
    model.model.to(device)
    model.model.eval()
    ...
    if args.fwt:
        range_adpt = len(model.task_list_seen)-1
    else:
        range_adpt = len(model.task_list_seen)


    if hparams.mask_infer or not args.single:   # mask infer/ToDCL
        s = model.smax if hparams.mask else None
        if TIL:
            # assert hparams.mask_infer
            for task_id in model.task_list_seen:
                if single_task and (task_id != model.task_list_seen[-1]):
                    continue
                t = model.task_list_seen.index(task_id)
                for idx_b, batch in tqdm(enumerate(test_loader[task_id]), total=len(test_loader[task_id])):
                # for idx_b, batch in enumerate(test_loader[task_id]):
                    with torch.no_grad():
                        value_batch, _ = test_generation_GPT2BATCH(model=model.model,
                                                                   tokenizer=model.tokenizer,
                                                                   input_text=[b + "[SOS]" for b in
                                                                               batch['history']],
                                                                   device=device,
                                                                   max_length=100,
                                                                   task_id=t,
            ...
            test_dataset = [test_datasets[task_id] for task_id in model.task_list_seen]
            ...
            for t in range(range_adpt):
                ...
                for task_id in model.task_list_seen:
                    ...
                    # for idx_b, batch in tqdm(enumerate(test_loader[task_id]), total=len(test_loader[task_id])):
                    for idx_b, batch in enumerate(test_loader[task_id]):
                        ppl_batch = model.compute_PPL(batch, task_id=t, device=device, s=s) ## one value per batch
            ...
            for k,v in perplexity_dict.items():
                if len(v) == range_adpt:
                    ...
                    print(k)    # logger.debug([k,v]) print(k,v)
            ...
            test_dataset_by_predicted_id = {k: make_val_loader(args, v, model.tokenizer) for k, v in
            ...
            for pred_task_id, task_loader in test_dataset_by_predicted_id.items():
        ...
        # for idx_b, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
        for task_id in model.task_list_seen:
            if args.mask and (use_all_masks is False) and (task_id != model.task_list_seen[-1]):    # 只测试当前mask在当前任务表现
                continue
            if single_task and (task_id != model.task_list_seen[-1]):
                continue
            for idx_b, batch in tqdm(enumerate(test_loader[task_id]), total=len(test_loader[task_id])):
                with torch.no_grad():
                    if masks is not None:
                        ...
                        value_batch, _ = test_generation_GPT2BATCH(model=model.model,
                                                                   ...
                                                                   input_text=[b + "[SOS]" for b in batch['history']],
                                                                   device=device,
                                                                   max_length=100)  # 未输入labels，则不返回loss，只返回output
                for idx, resp in enumerate(value_batch):
                    results.append({"id": batch["dial_id"][idx], "turn_id": batch["turn_id"][idx],
                                    "dataset": batch["dataset"][idx], "task_id": batch["task_id"][idx],
                                    "spk": batch["spk"][idx], "gold": batch["reply"][idx],
    ...
    with open(f'{args.saving_dir}/{time}'+f'/generated_responses{appendix}.json', 'w') as fp:
        json.dump(results, fp, indent=4)
    print(f'save to {time}/generated_responses{appendix}.json')
    tokenizer.padding_side = "right"
...
def test(hparams):
    ...
    model = Seq2SeqToD(hparams) # , hparams.saving_dir

    # hparams.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    _, _, dev_val_loader, (_, _, test_datasets) = get_data_loaders(hparams, model.tokenizer)
    # _, _, test_loader, (_, _) = get_data_loaders(hparams, tokenizer, test=True)
    ...
    if hparams.mask:
        if hparams.retrain:
            ...
            if hparams.mask_infer:
                test_model_seq2seq_ADAPTER(hparams, model, model.tokenizer, dev_val_loader, test_datasets,
                                           time=time, TIL=True, retrain=True)
