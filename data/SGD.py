...
def preprocessSGD(develop=False, multi_domain=False):
    ...
    data_by_multi_domain = defaultdict(list)
    for dial in data:
        if(len(dial["services"])==1):
            data_by_domain[str(sorted(dial["services"]))].append(dial)
        else:
            data_by_multi_domain[str(sorted(dial["services"]))].append(dial)
    ...
    data_by_multi_domain_new = defaultdict(list)
    for dom, data in data_by_multi_domain.items():
        train_data, dev_data, test_data = np.split(data, [int(len(data)*0.7), int(len(data)*0.8)])
        data_by_multi_domain_new[str(sorted([remove_numbers_from_string(s) for s in eval(dom)]))].append([train_data, dev_data, test_data])

    train_data, valid_data, test_data = [], [], []
    train_data_multi, valid_data_multi, test_data_multi = [], [], []
    ...
    for dom, list_of_split in data_by_multi_domain_new.items():
        train, valid, test = [], [], []
        for [tr,va,te] in list_of_split:
            train += rename_service_dialogue(tr,dom)
            valid += rename_service_dialogue(va,dom)
            test += rename_service_dialogue(te,dom)
        table.append({"dom":dom, "train":len(train), "valid":len(valid), "test":len(test)})
        train_data_multi += train
        valid_data_multi += valid
        test_data_multi += test

    if multi_domain:
        return train_data_multi, valid_data_multi, test_data_multi
    else:
